#!/usr/bin/env tsx
// Pre-launch readiness check. Verifies env, DB, CLI, storage dirs, marketing assets.
...
function color(result: Result, text: string): string {
  ...
  const ansi = result === "ok" ? "\x1b[32m" : result === "warn" ? "\x1b[33m" : "\x1b[31m";
...
if (!existsSync(DB_PATH)) {
  ...
  try {
    ...
    if (missing.length === 0) {
      record("database tables", "ok", `${tables.length} tables, all required present`);
    } else {
      record("database tables", "fail", `missing: ${missing.join(", ")}`);
    }
    db.close();
  } catch (error) {
...
if (existsSync(demoMp4))
  record("public/demo.mp4", "ok", `${Math.round(statSync(demoMp4).size / 1024)}KB`);
