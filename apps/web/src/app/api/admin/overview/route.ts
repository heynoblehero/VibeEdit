...
export async function GET() {
	...
	const totalRenders = db
		.select({ count: sql<number>`count(*)` })
		.from(renderJobs)
		.where(eq(renderJobs.status, "done"))
		.get()?.count ?? 0;

	const rendersLast24h = db
		.select({ count: sql<number>`count(*)` })
		.from(renderJobs)
		.where(
			and(eq(renderJobs.status, "done"), gte(renderJobs.createdAt, last24h)),
		)
		.get()?.count ?? 0;

	const failedRenders24h = db
		.select({ count: sql<number>`count(*)` })
		.from(renderJobs)
		.where(
			and(eq(renderJobs.status, "failed"), gte(renderJobs.createdAt, last24h)),
		)
		.get()?.count ?? 0;
	...
	const topUsers = db
		...
		.where(
			and(
				eq(usageEvents.kind, "chat_turn"),
				gte(usageEvents.createdAt, last30d),
			),
		)
