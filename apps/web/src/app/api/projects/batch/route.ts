...
const VARIANTS: Array<{ label: string; suffix: string }> = [
	...
	},
	{
		label: "Minimal",
		suffix:
			"Make this variant minimal — one idea per scene, generous negative space, no extra FX.",
	},
	{
		label: "Alt palette",
		suffix:
			"Make this variant with an unexpected color palette that still fits the niche.",
	},
];
...
export async function POST(req: Request) {
	...
	const count = Math.max(
		2,
		Math.min(MAX_VARIANTS, Number(body.variants) || 3),
	);
