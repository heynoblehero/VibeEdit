...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!resolvedOutput.startsWith(RENDERS_ROOT)) return new Response("not found", { status: 404 });
  if (!existsSync(resolvedOutput)) return new Response("file missing", { status: 404 });
