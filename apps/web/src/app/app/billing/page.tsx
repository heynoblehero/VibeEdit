    ...
    <main className="mx-auto max-w-5xl p-4 sm:p-8">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-3 sm:mb-10">
        ...
        <nav className="flex flex-wrap gap-3 text-sm">
          ...
          <Link href="/app/billing" className="text-[var(--color-accent)]">
            Billing
          </Link>
        </nav>
      </header>
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--color-accent)] bg-[var(--color-bg-2)] p-4">
          <div className="flex-1 text-sm">
            <div className="font-semibold text-[var(--color-fg)]">
              Your renders show a{" "}
              <span className="text-[var(--color-accent-2)]">VibeEdit watermark</span> and cap at
              720p.
            </div>
          ...
          >
            {busy === "creator" ? "Redirecting…" : "Upgrade to Creator · $19/mo"}
          </button>
        </div>
      )}
      ...
      <section className="mb-10 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-baseline gap-2 sm:gap-3">
              ...
              <span className="text-[var(--color-fg-muted)]">· {info.plan.priceLabel}/mo</span>
        ...
        {info.subscription.currentPeriodEnd && (
          <div className="mt-4 text-xs text-[var(--color-fg-muted)]">
            ...
            {info.subscription.cancelAtPeriodEnd && " — cancels at period end"}
          </div>
        )}
      </section>
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-baseline justify-between">
                ...
                {isCurrent && <span className="text-xs text-[var(--color-accent)]">Current</span>}
              </div>
              <div className="mb-3 text-3xl font-black">
                {plan.priceLabel}
                <span className="ml-1 text-sm font-normal text-[var(--color-fg-muted)]">/mo</span>
              </div>
              <ul className="mb-4 space-y-1 text-xs text-[var(--color-fg-muted)]">
                <li>Renders: {plan.renderLimit === -1 ? "Unlimited" : `${plan.renderLimit}/mo`}</li>
                <li>
                  Agent messages:{" "}
                  {plan.chatTurnLimit === -1 ? "Unlimited" : `${plan.chatTurnLimit}/mo`}
                </li>
