...
export default function EditorPage({ params }: PageProps) {
	...
	useEffect(() => {
		...
		fetch("/api/onboarding")
			...
			.then((data) => {
				if (cancelled) return;
				if (data && data.onboardingCompleted && !data.tourCompleted)
					setShowTour(true);
			})
			.catch(() => {
			...
			<header className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border)] bg-[var(--color-bg)] px-4 py-2 md:col-span-3">
				...
				<div className="flex flex-wrap items-center justify-end gap-2 text-xs">
					<button
						...
						className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-1.5 text-xs font-semibold text-[var(--color-fg)] hover:border-[var(--color-accent)]"
				...
				{(["chat", "preview", "files"] as MobileTab[]).map((tab) => (
					<button
						...
						className={`flex-1 py-3 text-sm font-medium capitalize ${
							mobileTab === tab
								? "border-t-2 border-[var(--color-accent)] bg-[var(--color-surface)] text-[var(--color-fg)]"
		...
		>
			...
			>
				...
				<p className="mb-3 text-xs text-[var(--color-fg-muted)]">
					Saves this project's current index.html into your personal
					snippet library at /app/snippets — fork it into new projects
					anytime.
				</p>
				<input
					autoFocus
