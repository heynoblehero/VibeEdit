		...
		<main className="mx-auto max-w-5xl p-4 sm:p-8">
			<header className="mb-8 flex flex-wrap items-center justify-between gap-3 sm:mb-10">
				...
				<div className="flex flex-wrap items-center gap-2 text-sm text-[var(--color-fg-muted)] sm:gap-3">
					...
					<Link
						href="/app/templates"
						className="hidden hover:text-[var(--color-fg)] sm:inline"
					>
						Templates
					</Link>
					<Link
			...
			<section className="mb-8 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
				<h2 className="mb-3 text-lg font-semibold">Start something new</h2>
				<div className="flex flex-wrap gap-2">
					<input
						value={name}
						onChange={(event) => setName(event.target.value)}
						placeholder="Project name (optional)"
						className="flex-1 min-w-[200px] rounded-md border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 outline-none focus:border-[var(--color-accent)]"
					/>
					<Link
						href="/app/templates"
					...
					<button
						...
						className="rounded-md border border-[var(--color-border)] px-4 py-2 hover:bg-[var(--color-bg)] disabled:opacity-50"
					>
						Empty project
					</button>
				</div>
							...
							<span className="flex flex-col items-end text-xs text-[var(--color-fg-muted)]">
								...
								{(p.renderCount ?? 0) > 0 && (
									<span className="text-[10px]">
										...
										{p.renderCount === 1 ? "" : "s"}
										{p.lastRenderAt
											? ` · last ${relativeTime(p.lastRenderAt)}`
											: ""}
									</span>
								)}
							</span>
