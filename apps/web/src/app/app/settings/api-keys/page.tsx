...
export default function ApiKeysSettingsPage() {
	...
	const [saved, setSaved] = useState<Record<ProviderId, string | null>>(
		emptyStringForm(),
	);
	...
	return (
		<main className="mx-auto max-w-3xl p-4 sm:p-8">
			...
			<p className="mb-6 max-w-2xl text-sm text-[var(--color-fg-muted)]">
				Paste your own keys to unlock image / video / voice generation
				inside the chat. Keys stay in <strong>this browser only</strong> —
				they never touch our database. The chat attaches them to each
				request and the server forwards directly to the provider.
			</p>

			<div className="mb-6 rounded-md border border-[var(--color-border)] bg-[var(--color-bg-2)] p-3 text-xs text-[var(--color-fg-muted)]">
				⚠ Re-paste keys on each new device or browser. Clearing site data
				wipes them.
			</div>
			...
			<section className="mt-8 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-5">
				...
				<ol className="space-y-2 text-sm text-[var(--color-fg)]">
					<li>
						<span className="font-semibold">1.</span> You ask the agent in
						chat: <em>"generate 4 background options for scene 2"</em>.
					</li>
					<li>
						<span className="font-semibold">2.</span> The agent calls
						Replicate / Kling with your key, saves the variants to{" "}
						<code className="rounded bg-[var(--color-bg)] px-1 text-xs">
							assets/variants/
						</code>
						.
					</li>
					<li>
						<span className="font-semibold">3.</span> The chat shows you a
						thumbnail grid. Click the winner — it becomes the scene's
						background, the rest stay as alternates.
					</li>
				</ol>
			</section>
		</main>
