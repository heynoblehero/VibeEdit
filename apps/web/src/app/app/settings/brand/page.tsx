	...
	return (
		<main className="mx-auto max-w-3xl p-4 sm:p-8">
			...
			<p className="mb-8 text-[var(--color-fg-muted)]">
				Upload once. The agent auto-applies these to every composition you
				create.
			</p>
			...
			<section className="mb-8 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-6">
				...
				<p className="mb-4 text-xs text-[var(--color-fg-muted)]">
					Describe your recurring host/presenter. The agent uses this to keep
					the character consistent across scenes and projects (illustrated
					archetype, pose, palette, energy).
				</p>
				<label className="mb-4 block">
					<span className="mb-1 block text-xs text-[var(--color-fg-muted)]">
						Host name
				...
				<label className="block">
					...
					<textarea
						...
						rows={4}
						onChange={(e) =>
							setKit({ ...kit, hostDescription: e.target.value })
						}
						onBlur={() =>
							save({ hostDescription: kit.hostDescription ?? "" })
						}
			...
			{saving && (
				<div className="text-xs text-[var(--color-fg-muted)]">Saving…</div>
			)}
		</main>
	);
}
