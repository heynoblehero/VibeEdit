...
export default function WorkerSettingsPage() {
	...
	async function revoke(prefix: string) {
		if (!confirm("Revoke this worker token? The worker will stop polling."))
			return;
	...
	return (
		<main className="mx-auto max-w-3xl p-4 sm:p-8">
			...
			<p className="mb-6 max-w-2xl text-[var(--color-fg-muted)]">
				Install the worker binary on your Mac, Windows, or Linux machine.
				Renders run locally — 2–5× faster than cloud, no queue waits.
			</p>
...
VIBEEDIT_TOKEN=vewk_... VIBEEDIT_URL=https://vibeedit.video npm start`}</pre>
				<p className="mt-3 text-xs text-[var(--color-fg-muted)]">
					Pre-built signed binaries for Mac / Win / Linux ship in week 4.
					See <Link href="/help/install-render-worker" className="underline">the docs</Link>.
				</p>
			</section>
				...
				<div>
					...
					{!hasWorker && !unlimited && (
						<div className="text-xs text-[var(--color-fg-muted)]">
							Cloud render time used: {cloudSecondsUsed}/{cloudSecondsLimit}{" "}
							seconds
						</div>
					)}
					{blocked && (
						<div className="mt-2 text-xs text-[var(--color-danger)]">
							Cap reached. Install the worker below to keep rendering, or
							upgrade your plan.
						</div>
					)}
				</div>
