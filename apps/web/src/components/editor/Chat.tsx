...
const REGENERATE_CHIPS: Array<{ label: string; prompt: string }> = [
	...
	},
	{
		label: "Tighter cuts",
		prompt:
			"Cut every scene duration ~20% tighter. Same beats, faster pace.",
	},
	{
		label: "Less text",
		prompt:
...
export function Chat({ projectId }: { projectId: string }) {
	...
	useEffect(() => {
		function onEditAt(event: Event) {
			...
			setInput((current) =>
				current.startsWith("Edit the scene at ") ? prefix : prefix + current,
			);
			setTimeout(() => {
		...
		function onEditAsset(event: Event) {
			...
			const prefix =
				detail.path === "the composition"
					...
					: `Edit \`${detail.path}\` — `;
			setInput((current) =>
				current.startsWith("Edit ") ? prefix : prefix + current,
			);
			setTimeout(() => {
				const node = textareaRef.current;
				if (!node) return;
	...
	async function dropFiles(fileList: FileList | File[]) {
		...
		try {
			...
			const uploaded = (data.uploaded || []).filter((path) =>
	...
	async function send(text?: string) {
		...
		const attachmentNote = hasAttachments
			? `Reference image${attachedAssets.length > 1 ? "s" : ""} attached at: ${attachedAssets
					.map((path) => `\`${path}\``)
					.join(", ")}. Use ${attachedAssets.length > 1 ? "them" : "it"} as visual reference (palette / composition / vibe).`
			: "";
		...
		>
					...
					<span className="flex items-center gap-1.5 rounded-full bg-[var(--color-bg-2)] px-2 py-0.5 text-[10px] text-[var(--color-accent)]">
						...
						<button
							onClick={() => {
								setEditingAt(null);
								setInput((current) =>
									current.startsWith("Edit the scene at ")
										? ""
										: current,
								);
							}}
			...
			<div className="border-t border-[var(--color-border)] p-3">
				...
				{busy ? (
					<button
						onClick={() => {
							fetch(`/api/chat?projectId=${projectId}`, { method: "DELETE" })
								.catch(() => {});
						}}
	...
	return (
		<div className="space-y-3">
			<div className="text-xs uppercase tracking-wider text-[var(--color-fg-muted)]">
				{prefs?.niche
					? `Starts for ${humanNiche(prefs.niche)}`
					: "Start with a prompt"}
			</div>
			<div className="grid grid-cols-1 gap-2">
				{ordered.map((sample) => (
					...
					>
						...
						<div className="line-clamp-2 text-xs text-[var(--color-fg-muted)]">
							{sample.prompt}
						</div>
					</button>
				))}
...
function LiveLine({
	...
	return (
		<div className="font-mono text-[var(--color-fg-muted)]">
			<span className={isDone ? "text-[var(--color-success)]" : ""}>
				{isDone ? "✓" : "→"}
			</span>{" "}
			...
			{entry.images && entry.images.length > 0 && (
				<ScreenshotStrip images={entry.images} />
			)}
			{entry.variants && projectId && (
				<VariantPicker info={entry.variants} projectId={projectId} />
			)}
		</div>
	);
}
	...
	async function pick(index: number) {
		...
		try {
			const response = await fetch(
				`/api/projects/${projectId}/variants/promote`,
				{
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({
		...
		<div className="rounded border border-[var(--color-border)] bg-[var(--color-surface)] font-mono">
			...
			>
				<span>
					<span
						className={
							allDone ? "text-[var(--color-success)]" : "text-[var(--color-accent)]"
						}
					>
						{allDone ? "✓" : "→"}
					</span>{" "}
				...
				<ol className="space-y-2 text-xs">
					{scenes.map((scene) => (
						...
						>
							...
							{scene.fx?.length > 0 &&
								!(scene.fx.length === 1 && scene.fx[0] === "none") && (
...
function summarizeToolInput(
	...
	return entries
		.map(([k, v]) => `${k}=${truncate(String(v).replace(/\s+/g, " "), 30)}`)
		.join(", ");
}
