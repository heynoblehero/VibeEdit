...
export function Preview({
	...
	const carryOverRef = useRef<{ time: number; wasPlaying: boolean } | null>(
		null,
	);
	...
	useEffect(() => {
		function onStatus(event: Event) {
			const detail = (event as CustomEvent<{ working: boolean; label?: string }>)
				.detail;
			if (!detail) return;
	...
	function onPlayerClick(event: React.MouseEvent<HTMLDivElement>) {
		...
		window.dispatchEvent(
			new CustomEvent("vibeedit:edit-at", { detail: { timestamp } }),
		);
	}
	...
	return (
		<div className="flex h-full w-full flex-col bg-black">
			<div className="flex items-center justify-between border-b border-[var(--color-border)] bg-[var(--color-bg)] px-4 py-2 text-xs text-[var(--color-fg-muted)]">
				<div className="flex items-center gap-3">
					...
					{lastUpdate && <Freshness ts={lastUpdate} />}
					{lastLatencyMs !== null && (
						<LatencyBadge last={lastLatencyMs} median={p50LatencyMs} />
					)}
				</div>
				<span className="font-mono">⌘P play · ⌘R render · ⇧click to edit frame</span>
			</div>
			<div
			...
			>
				...
				<div className="relative flex aspect-video w-full max-w-[min(100%,calc((70vh)*16/9))] items-center justify-center overflow-hidden rounded-lg border border-[var(--color-border)] bg-black">
	...
	return (
		<div className="flex max-w-md flex-col items-center justify-center p-6 text-center">
			...
			<div className="mt-2 text-sm text-white/60">
				Describe the video you want in the chat on the left. Your preview
				will render here once the agent builds it.
			</div>
		</div>
	);
}
	...
	return (
		<div className="flex max-w-md flex-col items-center justify-center p-6 text-center">
			...
			<div className="mt-3 text-xs text-white/50">
				First builds usually take 30–90s. Watch the chat for plan + tool
				calls.
			</div>
		</div>
	);
...
function Elapsed({
	...
	return (
		<div className="mt-3 font-mono text-[11px] text-[var(--color-fg-muted)]">
			elapsed {text}
		</div>
	);
}
...
function LatencyBadge({
	...
	const medianLabel =
		median !== null ? ` (p50 ${(median / 1000).toFixed(1)}s)` : "";
	return (
		<span
			className={
				overBudget ? "text-[var(--color-danger)]" : "text-[var(--color-success)]"
			}
