...
export function VariablesPanel({ projectId, reloadKey }: { projectId: string; reloadKey: number }) {
  ...
  function setValue(key: string, value: string) {
    setVars((prev) => prev.map((v) => (v.key === key ? { ...v, currentValue: value } : v)));
  }
      ...
      <div className="space-y-2">
        {vars.map((v) => (
          <div key={v.key} className="text-xs">
            <label className="mb-0.5 block text-[var(--color-fg-muted)]">{v.key}</label>
            <VarInput v={v} onChange={(value) => setValue(v.key, value)} />
          </div>
        ))}
      </div>
...
function VarInput({ v, onChange }: { v: Variable; onChange: (value: string) => void }) {
  if (v.type === "color") {
    return (
