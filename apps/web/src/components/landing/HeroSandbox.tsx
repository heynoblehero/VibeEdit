...
type Niche = "comic" | "anime" | "scifi" | "history" | "finance" | "sleep" | "scary" | "tech";
...
const NICHES: Record<Niche, NicheSpec> = {
  comic: {
    ...
    gradient: "radial-gradient(circle at 50% 30%, #ffd43b 0%, #ff2b3a 60%, #1a0608 100%)",
    dots: true,
    headline: "COMIC",
  ...
  scifi: {
    ...
    gradient: "radial-gradient(circle at 50% 40%, #00ddff30 0%, #050a18 60%, #000 100%)",
    headline: "[FILE 087]",
  ...
  history: {
    ...
    gradient: "radial-gradient(circle at 50% 30%, #c89b58 0%, #6e4023 50%, #1e0e05 100%)",
    headline: "ROANOKE",
  ...
  scary: {
    ...
    gradient: "radial-gradient(circle at 50% 60%, #1c2230 0%, #050810 70%, #000 100%)",
    headline: "DON'T",
...
const EXAMPLE_PROMPTS: Array<{ label: string; prompt: string }> = [
  {
    label: "Trim & caption",
    prompt: "Trim my talking head, remove filler words, auto-grade, burn 2-word captions.",
  },
  {
    label: "Interview reel",
    prompt: "Cut my interview into a 60s highlight reel. Normalize audio for Instagram.",
  },
  {
  ...
  {
    label: "Anime Short",
    prompt: "25s anime facts Short. Pink + cyan, speed lines, chromatic title with scale-pulse.",
  },
  {
    label: "History mystery",
    prompt: "45s 16:9 history mystery intro about Roanoke. Sepia, slow ken-burns, candle flicker.",
  },
  {
    label: "Finance hook",
    prompt:
  ...
  {
    label: "Sleep story",
    prompt: "60s 16:9 sleep-story intro about ancient stars. Indigo + amber, slow, no flashes.",
  },
  {
    label: "Scary story",
    prompt:
...
type Phase = "idle" | "thinking" | "plan" | "approved" | "building" | "preview" | "done";
...
export function HeroSandbox() {
  ...
  const showPlan =
    ...
    phase === "done";
  const showApproved =
    phase === "approved" || phase === "building" || phase === "preview" || phase === "done";
  const showBuild = phase === "building" || phase === "preview" || phase === "done";
    ...
    <div className="relative">
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-2xl">
        <div className="flex items-center gap-2 border-b border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2">
          ...
          <span className="ml-auto flex items-center gap-1.5 font-mono text-[10px] text-[var(--color-fg-muted)]">
            ...
            />
            {phase === "idle" ? "waiting" : showPreview ? "ready" : "working"}
          </span>
        </div>
            ...
            {phase === "idle" ? (
              <>
                ...
                <p className="text-[10px] text-[var(--color-fg-muted)]">
                  This is a live sandbox. Real account, real render, real MP4 → after sign-up.
                </p>
              </>
            ) : (
              <>
                ...
                {showPlan && (
                  <PlanCard niche={niche} duration={activeDuration} format={activeFormat} />
                )}
                {phase === "plan" && <AgentLine>Approve this plan and I'll build it.</AgentLine>}
                {showApproved && <UserBubble>yes go</UserBubble>}
                {showBuild && (
                  <BuildLog shownTools={shownTools} done={showPreview} musicMood={niche.music} />
                )}
                {phase === "done" && <AgentLine accent>Built it. Render?</AgentLine>}
          ...
          <div className="bg-black p-4">
            <div className="mb-2 flex items-center justify-between text-[10px] text-[var(--color-fg-muted)]">
              <span className="flex items-center gap-1.5">
                <span
                  className={`inline-block h-1.5 w-1.5 rounded-full ${
                    showPreview ? "bg-[var(--color-success)]" : "bg-[var(--color-fg-muted)]"
                  }`}
                />
      ...
      {phase !== "idle" && (
        <div className="mt-3 flex items-center justify-center gap-3 text-[10px] text-[var(--color-fg-muted)]">
          ...
          <Dot active={phase === "plan" || phase === "approved"} label="plan" />
          <Dot active={phase === "building"} label="build" />
          <Dot active={phase === "preview" || phase === "done"} label="preview" />
        </div>
      )}
    </div>
  );
...
function nextPhase(current: Phase): Phase {
  const order: Phase[] = ["idle", "thinking", "plan", "approved", "building", "preview", "done"];
...
function AgentLine({ children, accent }: { children: React.ReactNode; accent?: boolean }) {
  return (
    <div
    ...
    <div className="overflow-hidden rounded-lg border border-[var(--color-accent)] bg-[var(--color-bg)] text-[10px] animate-[fadeIn_280ms_ease-out]">
      ...
      <ol className="space-y-1 p-2 text-[var(--color-fg)]">
        {niche.scenes.map((scene, index) => (
          <li key={scene}>
            <span className="text-[var(--color-accent)]">#{index + 1}</span> {scene}
          </li>
        ))}
      </ol>
    </div>
  ...
  return (
    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-2)] p-2 font-mono text-[10px]">
      <div className="mb-1 flex items-center gap-1.5">
        <span className={done ? "text-[var(--color-success)]" : "text-[var(--color-accent)]"}>
          {done ? "✓" : "→"}
        </span>
        <span className="text-[var(--color-fg)]">{done ? "Built it" : "Building"}</span>
...
function PreviewPane({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`font-black leading-none ${format === "9:16" ? "text-5xl" : "text-6xl"}`}
          style={{
