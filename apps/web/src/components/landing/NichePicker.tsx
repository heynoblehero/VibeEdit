...
const NICHES: Niche[] = [
  ...
  },
  {
    id: "anime",
    label: "Anime facts",
    prompt: "25s anime facts Short. Pink + cyan, speed lines, chromatic title, scale-pulse.",
  ...
  },
  {
    id: "history",
    label: "History",
    prompt: "45s 16:9 history mystery intro. Sepia, slow ken-burns, candle flicker, no flashes.",
  ...
  },
  {
    id: "finance",
    label: "Finance",
    prompt: "20s finance hook. Black + neon green, animated counters, ticker tape, scanlines.",
  ...
  },
  {
    id: "scary",
    label: "Scary stories",
    prompt: "30s horror hook. Dark blue + sickly green, vignette, low rumble, glitch on title.",
  ...
  },
  {
    ...
    prompt: "15s tech tutorial intro. Dark UI, terminal-green accent, code rain, animated counter.",
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:gap-10 lg:items-center">
      <div className="order-2 lg:order-1">
        ...
        <div className="mt-3 text-sm text-[var(--color-fg-muted)]">{active.subline}</div>
      </div>
        ...
        <div className="relative mx-auto aspect-[9/16] w-full max-w-[260px] overflow-hidden rounded-2xl ring-1 ring-white/10 transition-all duration-500">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/60 px-3 py-2 backdrop-blur-sm">
            ...
            <span className="rounded bg-white/15 px-1.5 py-0.5 font-mono text-[10px] uppercase text-white/80">
              {active.id === "history" || active.id === "finance" || active.id === "tech"
                ? "16:9"
                : "9:16"}
            </span>
          </div>
