...
export async function replicateGenerateImage(
	...
	if (!startResponse.ok) {
		...
		throw new Error(
			`replicate start failed: ${startResponse.status} ${text.slice(0, 400)}`,
		);
	}
	...
	while (
		...
		if (!pollResponse.ok) {
			...
			throw new Error(
				`replicate poll failed: ${pollResponse.status} ${text.slice(0, 400)}`,
			);
		}
	...
	if (!imageResponse.ok) {
		throw new Error(
			`replicate image fetch failed: ${imageResponse.status}`,
		);
	}
