...
async function probeReplicate(apiKey: string): Promise<ProbeResult> {
	try {
		...
		if (!response.ok)
			return { ok: false, error: `replicate ${response.status}` };
...
async function probeElevenLabs(apiKey: string): Promise<ProbeResult> {
	try {
		...
		if (!response.ok)
			return { ok: false, error: `elevenlabs ${response.status}` };
		const data = (await response.json().catch(() => ({}))) as {
			subscription?: {
				tier?: string;
...
async function probeOpenAi(apiKey: string): Promise<ProbeResult> {
	try {
		...
		const detail = data.data?.length
			? `${data.data.length} models accessible`
			: "ok";
		return { ok: true, detail };
	} catch (caught) {
		return { ok: false, error: (caught as Error).message };
...
async function probeAnthropic(apiKey: string): Promise<ProbeResult> {
	try {
		...
		if (response.status === 401 || response.status === 403)
			return { ok: false, error: "invalid key" };
		if (!response.ok)
			return { ok: false, error: `anthropic ${response.status}` };
		const data = (await response.json().catch(() => ({}))) as {
			data?: Array<{ id: string }>;
		};
		const detail = data.data?.length
