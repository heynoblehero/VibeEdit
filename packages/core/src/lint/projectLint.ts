 ...
 * in compositions/, and runs cross-file checks (missing/duplicate audio,
 * texture-mask assets, multiple root compositions). Pure filesystem + string
 * analysis — no browser, no CLI deps — so it can run in-process anywhere the
 * project directory is readable.
 *
 * Relocated from the CLI's `utils/lintProject.ts` so the web app's agent can
 * lint compositions without spawning the `hyperframes` CLI. The CLI keeps a
 * thin `ProjectDir`-accepting wrapper around this for back-compat.
 */
 ...
 */
export function lintProject(projectDir: string): ProjectLintResult {
